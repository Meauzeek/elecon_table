...
function applyFilter(){
 ...
 if(mode==='off')legend.innerHTML='<span><i class="dot active"></i>已有档案</span><span><i class="dot vacant"></i>未观测</span><span><i class="dot special"></i>特殊序号</span><span><b class="fold-key"></b> 右上角折叠标记可展开关联个体</span>';
 else legend.innerHTML=Object.entries(palette).map(([k,c])=>`<span><i class="dot" style="background:${c}"></i>${esc(k)}</span>`).join('');
}
const GROUP_FILTER_ORDER=['爱伊忒拿体系','CUA / 南洲教育体系','埃尔芮欧家族','LLNL','LUACL','PASC','Apëiron','天平协定','WUI','南极科研 / 航天体系','其他组织','独立 / 无 / 未详'];
function groupCategories(id,p){const raw=groupFor(id,p),cats=[];const add=x=>{if(!cats.includes(x))cats.push(x)};const aeternaIds=['el:C','el:N','el:In','el:Rb','el:Pu','off:EN','off:DD','off:VN','off:SI','off:DC','off:GX'];if(/爱伊忒拿|陵墓|Juno/.test(raw)||aeternaIds.includes(id))add('爱伊忒拿体系');if(/CUA|南洲大附中|南极洲立大学|阿尔西亚课题组/.test(raw))add('CUA / 南洲教育体系');if(['el:F','el:V','off:AS','off:OA','off:TB'].includes(id))add('埃尔芮欧家族');if(/LLNL/.test(raw)||['el:W','el:U','off:WJ'].includes(id))add('LLNL');if(/LUACL/.test(raw))add('LUACL');if(/PASC/.test(raw))add('PASC');if(/Apëiron|Apeiron/.test(raw))add('Apëiron');if(/天平协定/.test(raw))add('天平协定');if(/WUI/.test(raw))add('WUI');if(/南极航空航天局/.test(raw))add('南极科研 / 航天体系');if(!cats.length&&/无|独立|未详/.test(raw))add('独立 / 无 / 未详');if(!cats.length)add('其他组织');return cats}
function populateGroupFilter(){if(!groupSelect)return;const previous=groupSelect.value||'all',used=new Set();for(const [id,item] of REGISTRY){if(item.redirect||!item.p||id.startsWith('der:'))continue;groupCategories(id,profileWithEdits(id,item.p)).forEach(g=>used.add(g))}const groups=GROUP_FILTER_ORDER.filter(g=>used.has(g));groupSelect.innerHTML='<option value="all">全部团体</option>'+groups.map(g=>`<option value="${esc(g)}">${esc(g)}</option>`).join('');groupSelect.value=[...groupSelect.options].some(o=>o.value===previous)?previous:'all'}
function applyGroupFilter(){if(!groupSelect)return;const selected=groupSelect.value;document.querySelectorAll('[data-profile].element-card,[data-profile].off-card,.special-card').forEach(el=>{const id=el.dataset.profile,item=REGISTRY.get(id);if(!item||item.redirect)return;const groups=groupCategories(id,profileWithEdits(id,item.p));el.classList.toggle('group-hidden',selected!=='all'&&!groups.includes(selected));el.dataset.group=groups.join('、')})}
